...
export default async function TeamPerformance() {
  ...
  const memberPerformance = user.leadingTeam.members.map(member => ({
    ...
    averageResolutionTime: member.assignedCases.reduce((acc, curr) => 
      acc + (curr.updatedAt.getTime() - curr.createdAt.getTime()), 0) / member.assignedCases.length / (1000 * 60 * 60) // in hours
  }));
